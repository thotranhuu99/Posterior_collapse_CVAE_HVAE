...
def main(args):
    ...
    dataset_test = MNISTDatasetCVEQuarter(root='./data', train=False)
    ...
    model = CVAE(dim_x=args.dim_x, dim_y=args.dim_y, dim_z=args.dim_z, d_hidden=args.d_hidden,
                 eta_enc = args.eta_enc, eta_dec=args.eta_dec, dataset=dataset, beta=args.beta,
    ...
    for epoch in pbar:
        ...
        for batch_idx, (x, y, label) in enumerate(dataloader):
            ...
            optimizer.step()
            pbar.set_description("Loss: {:.12f}, NLL : {:.6f}".format(loss, nll_p_y_given_x))
    ...
    with torch.no_grad():
        ...
        nll_p_y_given_x_mean = nll_p_y_given_x_total / len(dataset_test)
        nll_p_y_given_x_z_mean = nll_p_y_given_x_z_total / len(dataset_test)

    print("NLL is {}".format(nll_p_y_given_x_mean))

    with torch.no_grad():
        model.eval()
        loss_KL_perdim_all = torch.zeros([len(dataset_test), model.dim_z], device="cuda")
        for batch_idx, (x, y, label) in enumerate(evalloader):
            x, y = x.to("cuda"), y.to("cuda")
            y_parameterized, mu_z_enc, sigma, mu_y = model(x, y)
            loss, loss_elements, loss_KL_perdim = model.loss_fn(y_parameterized, mu_z_enc, sigma, mu_y, y)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_folder', type=str, default="output/performance/cvae_nonlinear/temp/image")
    parser.add_argument('--json_folder', type=str, default="output/performance/cvae_nonlinear/temp/json")
    parser.add_argument('--theta_path', type=str, default="output/performance/cvae_nonlinear/temp/")
    parser.add_argument('--npy_path', type=str, default="output/performance/cvae_nonlinear/temp/npy")
